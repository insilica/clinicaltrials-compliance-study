...
create_logranks.pairwise <- function(agg.window) {
  combined_data <- agg.window |>
    ...
    bind_rows()
  #list(
  #  logrank.funding       = pairwise_survdiff(Surv(surv.time_months, surv.event) ~ window_name + common.funding,
  #                                            data = combined_data),
  #  logrank.phase         = pairwise_survdiff(Surv(surv.time_months, surv.event) ~ window_name + common.phase.norm,
  #                                            data = combined_data),
  #  logrank.interventions = pairwise_survdiff(Surv(surv.time_months, surv.event) ~ window_name + common.intervention_type,
  #                                            data = combined_data),
  #  logrank.status        = pairwise_survdiff(Surv(surv.time_months, surv.event) ~ window_name + common.overall_status,
  #                                  data = combined_data)
  #)
  ...
  #    group_modify(~broom::tidy(survdiff(Surv(surv.time_months, surv.event) ~ window_name, data = .x))),
  ...
  #  logrank.funding = combined_data |>
  #    group_by(common.funding) |>
  #    group_map(~survdiff(Surv(surv.time_months, surv.event) ~ window_name, data = .x)),
  #  logrank.phase = combined_data |>
  #    group_by(common.phase.norm) |>
  #    group_map(~survdiff(Surv(surv.time_months, surv.event) ~ window_name, data = .x)),
  #  logrank.interventions = combined_data |>
  ...
  #    group_by(common.intervention_type) |>
  #    group_map(~setNames(list(survdiff(Surv(surv.time_months, surv.event) ~ window_name, data = .x)),
  #                       .y$common.intervention_type)) |>
  #    unlist(recursive = FALSE),

  #  logrank.status = combined_data |>
  #    group_by(common.overall_status) |>
  #    group_map(~setNames(list(survdiff(Surv(surv.time_months, surv.event) ~ window_name, data = .x)),
  #                       .y$common.overall_status)) |>
  #    unlist(recursive = FALSE)
  #)
